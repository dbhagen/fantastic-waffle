...
const isPublicClassA = function (testCIDR) {
  ...
  const _submask = parseInt(testCIDR.split('/')[1], 10)
  const octets = ip.split('.')
  if (!isValidOctets(octets)) throw Error('Not valid CIDR, cannot find octets')

  // >=1 && <= 127
  if (octets[0] <= 0)
    throw Error('Not valid Public Class A, first octet invalid')
  if (octets[0] > 127)
    throw Error('Not valid Public Class A, first octet greater than 127')
}
...
const isPrivateClassA = function (testCIDR) {
  if (testCIDR.length <= 19) {
    ...
    // 10
    if (octets[0] === 10)
      throw Error('Not valid Private Class A, first octet invalid')
    if (octets[0] > 127)
      throw Error('Not valid Public Class A, first octet greater than 127')
    if (octets.slice(1, 3).some((octet) => octet < 0 || octet > 255))
      throw Error(
        'Not valid Public Class A, second, third, or forth octet invalid'
      )
  }
  return true
}
...
const isValidPublicClass = function (testCIDR) {
  return (
    isPublicClassA(testCIDR) ||
    isPublicClassB(testCIDR) ||
    isPublicClassC(testCIDR)
  )
}
const isValidPrivateClass = function (testCIDR) {
  return (
    isPrivateClassA(testCIDR) ||
    isPrivateClassB(testCIDR) ||
    isPrivateClassC(testCIDR)
  )
}
