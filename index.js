...
const clusterName = pulumiConfig.get('clusterName')
  ? pulumiConfig.get('clusterName')
  : 'eks-i'
const clusterDesiredSize = pulumiConfig.get('clusterDesiredSize')
  ? parseInt(pulumiConfig.get('clusterDesiredSize'), 10)
  : 2
const clusterMinSize = pulumiConfig.get('clusterMinSize')
  ? parseInt(pulumiConfig.get('clusterMinSize'), 10)
  : 1
const clusterMaxSize = pulumiConfig.get('clusterMaxSize')
  ? parseInt(pulumiConfig.get('clusterMaxSize'), 10)
  : 2
const clusterVpcCIDR = pulumiConfig.get('clusterVpcCIDR')
  ? pulumiConfig.get('clusterVpcCIDR')
  : '10.0.0.0/16'
const deploymentName = pulumiConfig.get('deploymentName')
  ? pulumiConfig.get('deploymentName')
  : 'nginx'
const deploymentImage = pulumiConfig.get('deploymentImage')
  ? pulumiConfig.get('deploymentImage')
  : 'nginx:latest'
const deploymentPort = pulumiConfig.get('deploymentPort')
  ? parseInt(pulumiConfig.get('deploymentPort'), 10)
  : 80
const deploymentReplicas = pulumiConfig.get('deploymentReplicas')
  ? parseInt(pulumiConfig.get('deploymentReplicas'), 10)
  : 1
const projectName = pulumiConfig.projectName
  ? pulumiConfig.projectName
  : 'fantastic-waffle'
...
exports.kubeconfig = pulumi.secret(
  eksCluster.kubeconfig.apply((kc) => {
...
const loadBalancerServiceHostname = pulumi.secret(
  loadBalancerService.status.apply((s) => s.loadBalancer.ingress[0].hostname)
)
